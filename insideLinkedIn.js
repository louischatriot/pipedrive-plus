...
for (i = 0; i < experienceHeaders.length; i += 1) {
  ...
  $header = $(experienceHeaders[i]);
  $parent = $header.parent();
  $date = $parent.find('span.experience-date-locale');

  $h5 = $header.find('h5');
  if ($h5.length === 1) {
    workDatum.company = $h5.find('a').html();
  } else {
    for (j = 0; j < $h5.length; j += 1) {
      if (! $($h5[j]).hasClass('experience-logo')) {
        workDatum.company = $($h5[j]).find('a').html();
      }
    }
  }

  workDatum.date = $date.html().replace(/<time>/g, '').replace(/<\/time>/g, '').replace(/<.*>/, '');

  workData.push(workDatum);
}
...
// Action tab
var $actionPane = $('<div id="actionPane" style="position:fixed; top: 0px; right: 0px; bottom: 0px; left: 100%; background-color: #ddd;"></div>');
